...
- Token bucket (supports burst allowances)
...
// The middleware factory returns wrapped setRequestHandler
const mt = createMultiTenantMiddleware({
  tenantResolver: new JWTTenantResolver({ claim: "tenant_id" }),
  rateLimiter: new RateLimiter(new RedisRateLimitStore()),
  toolVisibility: new ToolVisibilityEngine(policies),
  costAccounting: new CostAccountant(pricing),
  artifactStore: new S3ArtifactStore(bucket),
  configStore: new DatabaseConfigStore(connection)
});
...
2. Resolves tenant context (connection-scoped, cached after `initialize`)
...
const config: MultiTenantConfig = {
  tenantResolver: { type: "jwt", claim: "tenant_id" },
  rateLimit: {
    store: "redis",
    default: { requestsPerMinute: 100, tokensPerMinute: 10000 }
  },
  toolVisibility: { store: "database" },
  costAccounting: { enabled: true, pricing: {...} },
  artifactStore: { type: "s3", bucket: "..." },
  configStore: { type: "database" }
};
```
